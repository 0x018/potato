...
var users = new Map<string, WebSocket>();

 // 广播; 散布，传播
 export function broadcast(message: any, id?: any[]): void {
  if (!message) return;
  console.log("broadcast",message,id);
...
export async function chat(ws: WebSocket): Promise<void> {
  const userId = 'dev';// Math.floor(10 * Math.random());// v4.generate();
